...
[s3fs-fuse](https://github.com/s3fs-fuse/s3fs-fuse) is a FUSE (File System in Userspace) filesystem that allows you to mount an S3 bucket as a local filesystem. It is written in C++ and uses the libfuse library to communicate with the Linux kernel and interact with the filesystem.
...
:::info
To access the actual guide how to mount a bucket with s3fs-fuse [**click here**](./mount-s3-storage)!
:::
